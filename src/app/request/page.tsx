...
import { addRequest } from "../api/seller/addRequest";
...
export default function RequestPage() {
  ...
  const handleRequest = async () => {
    setLoading(true);
    try {
      const response = await addRequest(formData, imageUrl);
      if (response) {
        setIsSent(true);
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-6xl">
      <h1 className="text-3xl font-bold mb-8 text-center">Request a Product</h1>

      <form className="space-y-6">
        <div className="">
          <p className="text-red-500"><span className="font-bold text-gray-500">Note: </span> <span>After submitting the request, Admin will approve it and then it will be listed on the Inquiry page.</span></p>
        </div>
      ...
      {isSent && (
        <div className="fixed top-0 left-0 right-0 z-50 flex items-center justify-center w-screen h-screen bg-black/50">
          <div className="bg-white p-6 rounded-lg shadow-lg">
            <h2 className="text-lg font-semibold mb-4">Request Sent!</h2>
            <p className="mb-4 text-center max-w-xl mx-auto">
              Your request has been sent successfully. Once Admin approves it, it will be listed on the Inquiry page.
            </p>
